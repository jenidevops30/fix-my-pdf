    ...
    case "working":
      return (
        <Badge className="bg-slate-100 text-slate-700 border-transparent">
          <span className="size-1.5 rounded-full bg-slate-700 animate-pulse" aria-hidden="true" />
          Working…
        </Badge>
      );
    case "fit":
      return result.result.success ? (
        ...
        <Badge className="bg-rose-100 text-rose-700 border-transparent">Closest Match Found</Badge>
      );
    case "pages":
      return (
        <Badge className="bg-emerald-100 text-emerald-800 border-transparent">Cut Complete</Badge>
      );
    case "error":
      return <Badge className="bg-rose-100 text-rose-700 border-transparent">Error</Badge>;
  }
}
...
function PassRow({ pass, emphasized }: { pass: CompressionPass; emphasized: boolean }) {
  return (
    ...
    >
      <span className={cn("truncate", emphasized ? "text-slate-900" : "text-slate-600")}>
        {pass.label}
        {pass.note && <span className="text-[10px] text-slate-400"> · {pass.note}</span>}
      </span>
      <span className="flex items-center gap-1.5 shrink-0">
        {Number.isNaN(pass.size) ? (
          <span className="text-rose-500">failed</span>
        ) : (
          <span className={emphasized ? "text-slate-900" : "text-slate-700"}>
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-2">
        ...
        <div className="h-2 bg-slate-200/80 rounded-full overflow-hidden" aria-hidden="true">
      ...
      <div className="border-t border-slate-200/80 pt-4 space-y-1.5 font-mono text-xs">
      ...
      {!res.success && (
        <p className="rounded-lg bg-amber-50 border border-amber-200 text-amber-800 p-3 text-xs">
  ...
  return (
    <div className="space-y-5">
      <ul className="space-y-1.5">
        {r.bullets.map((b) => (
          <li key={b} className="flex items-start gap-2 text-xs font-mono text-slate-600">
            <Check className="size-3.5 text-emerald-600 mt-0.5 shrink-0" aria-hidden="true" />
            <span className="break-all">{b}</span>
          </li>
        ))}
      </ul>
      <div className="rounded-lg bg-white border border-slate-200/80 p-5 text-center">
        <div className="text-4xl font-extrabold">{r.pagesOut}</div>
        <div className="text-xs text-slate-500 mt-1">pages in the new file</div>
      </div>
    </div>
  );
}
...
export function ResultPanel({ result, mode, onReset }: ResultPanelProps) {
  ...
  return (
    <aside
      ...
      aria-live="polite"
      className="rounded-xl bg-[#f8faff] border border-slate-200/80 p-6 sm:p-7 flex flex-col justify-between gap-6 min-h-[420px]"
    >
      <div className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-1.5">
              {IDLE_PROMISES[mode].map((line) => (
                ...
                >
                  <span
                    className="mt-1.5 size-1 rounded-full bg-slate-400 shrink-0"
                    aria-hidden="true"
                  />
                  {line}
                </li>
        ...
        {result.kind === "working" && (
          <div className="space-y-2">
            ...
            />
            <p className="font-mono text-xs text-slate-600">{result.phase}</p>
            {result.detail && <p className="text-[11px] text-slate-400">{result.detail}</p>}
          </div>
        )}
        ...
        {result.kind === "error" && (
          <div className="space-y-4">
            <div
              role="alert"
              className="rounded-lg bg-rose-50 border border-rose-200 p-4 text-sm text-rose-800"
            >
              {result.message}
            </div>
